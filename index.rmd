...
The `practical_zip.zip` file you have just downloaded is a `.zip` file, which is just a way of compressing all the data to create a file with a smaller filesize. Before doing anything else, we need to first unzip it to retain the original files.  If you right click on the `practical.zip` that you have downloaded, you will have a list of options, and you need to select `Extract all`. By default it will extract all the files to the directory that the zip file is in: this is fine, but if you would like to save the files somewhere in particular, you can also specify a different directory at this stage. When it has done this you should be able to see a practical_zip folder that, unlike the zipped version, no longer has a zip across the thumbnail. 
